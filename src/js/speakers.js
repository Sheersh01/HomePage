...
document.addEventListener("DOMContentLoaded", () => {
  ...
  speakersData.forEach((speaker, index) => {
    const imgSrc = speakerImages[index]; // use imported image
          ...
          speaker.linkedin
            ? `<li>
                 <a href="${speaker.linkedin}" rel="noreferrer" target="_blank">
                   <img alt="LinkedIn profile link" src="${linkIcon}" class="w-8 h-8 rounded-md transition-transform duration-300 hover:scale-110"/>
                 </a>
               </li>`
            : ""
        }
          ...
          speaker.instagram
            ? `<li>
                 <a href="${speaker.instagram}" rel="noreferrer" target="_blank">
                   <img alt="Instagram profile link" src="${instaIcon}" class="w-8 h-8 rounded-md transition-transform duration-300 hover:scale-110"/>
                 </a>
               </li>`
            : ""
        }
          ...
          speaker.website
            ? `<li>
                 <a href="${speaker.website}" rel="noreferrer" target="_blank">
                   <img alt="Website link" src="../assets/website-icon.svg" class="w-8 h-8 rounded-md transition-transform duration-300 hover:scale-110"/>
                 </a>
               </li>`
            : ""
        }
    ...
    card.innerHTML = `
      ...
      <div class="relative z-10 p-6 flex flex-col items-center text-center h-full">
        <div class="relative w-36 h-36 rounded-full mt-2 mb-4 transition-all duration-1000 group-hover:scale-105">
          <img src="${imgSrc}" alt="${speaker.name}" class="w-full h-full object-cover rounded-full transition-all duration-1000 group-hover:filter group-hover:brightness-110 group-hover:contrast-110"/>
